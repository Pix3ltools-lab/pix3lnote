...
export function NoteEditor({ note, onClose }: NoteEditorProps) {
  const {
    updateNote, deleteNote, toggleArchive, addLabelToNote, removeLabelFromNote, labels,
  ...
  const isOwner = note?.access === 'owner';
  const canEdit = note?.access === 'owner' || note?.access === 'editor';
  ...
  const handleSave = async () => {
    if (!note || !hasChanges || !canEdit) return;
    setSaving(true);
  ...
  const handleAddChecklistItem = async (text: string) => {
    if (!note) return;
    const item = await addChecklistItem(note.id, text);
  ...
  const handleToggleChecklistItem = (itemId: string, checked: boolean) => {
    ...
    updateChecklistItem(note.id, itemId, { checked });
  };

  const handleChangeChecklistItemText = (itemId: string, text: string) => {
    if (!note) return;
    setChecklist(prev => prev.map(i => i.id === itemId ? { ...i, text } : i));
    updateChecklistItem(note.id, itemId, { text });
  };
  ...
  return (
    <Modal isOpen onClose={handleClose} maxWidth="max-w-xl">
      ...
      >
        {note && note.access !== 'owner' && (
          <p className="px-4 pt-3 text-xs opacity-60">
            Shared by {note.owner_name} · {note.access === 'editor' ? 'can edit' : 'view only'}
          </p>
        )}
        ...
        {note && (checklist.length > 0 || canEdit) && (
          <ChecklistSection
            items={checklist}
            canEdit={canEdit}
            onAddItem={handleAddChecklistItem}
            onToggleItem={handleToggleChecklistItem}
            onChangeItemText={handleChangeChecklistItemText}
        ...
        {showLabelPicker && isOwner && labels.length > 0 && (
        ...
        <div className="flex items-center justify-between border-t border-black/5 px-3 py-2">
          <div className="flex items-center gap-1">
            ...
            {isOwner && (
              <ToolButton onClick={() => setShowShareModal(true)} title="Share">
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {note && showShareModal && (
        <ShareModal noteId={note.id} onClose={() => setShowShareModal(false)} />
      )}
    </Modal>
  );
}
